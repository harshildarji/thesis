...
for mode in modes:
    ...
    plt.title(
        "{} - Correlations between different graph properties and test results".format(
            mode
        )
    )
    ...
    for c in cols:
        ...
        sns.jointplot(
            ...
            kind="reg",
            height=10,
            xlim=(file["test_acc"].min() - 0.04, 1.0),
        ).plot_joint(
            sns.kdeplot, zorder=0, n_levels=5, alpha=0.2, color="k", shade=False
        )
        plt.figtext(0.32, 0, "pearsonr = {:.2f}; p = {:.2f}".format(pearson, p))
        plt.suptitle("Jointplot for test_acc and {}".format(c), y=1.01)
        plt.savefig(
            "{}/test_acc_jointplots/jointplot_test_acc_{}.png".format(mode, c),
            bbox_inches="tight",
            pad_inches=0.1,
        )
        plt.close()
    ...
    sns.jointplot(
        ...
        edgecolor="w",
        linewidth=1,
    ).plot_joint(sns.kdeplot, zorder=0, n_levels=5, alpha=0.2, color="k", shade=True)
    plt.suptitle("Jointplot for nodes and layers".format(c), y=1.01)
    plt.savefig(
        "{}/jointplot_nodes_layers.png".format(mode),
        bbox_inches="tight",
        pad_inches=0.1,
    )
    plt.close()
