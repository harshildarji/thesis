...
def plot_prune(group, title, regain_title):
    ...
    prune_test_acc = [
        float(prune.get_group(percent)["test_acc"].iloc[0]) for percent in PERCENT
    ]
    ...
    plt.legend()
    plt.savefig(
        "{}/{}_{}.png".format(mode, mode.lower(), title.lower().replace(" ", "_"))
    )
    plt.close()
    ...
    for percent in PERCENT:
        ...
        threshold = 0.92 if "tanh" in group else 0.95
    ...
    plt.savefig(
        "{}/{}_{}.png".format(
            mode, mode.lower(), regain_title.lower().replace(" ", "_")
        )
    )
    plt.close()
...
for mode in MODES:
    ...
    plot_prune(
        "{}_prune_i2h".format(mode.lower()),
        "I2H Pruning Evaluation",
        "I2H Accuracy Regain",
    )
    plot_prune(
        "{}_prune_h2h".format(mode.lower()),
        "H2H Pruning Evaluation",
        "H2H Accuracy Regain",
    )
