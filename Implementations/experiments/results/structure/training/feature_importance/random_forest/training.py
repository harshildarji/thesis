...
if __name__ == "__main__":
    ...
    for feature, feature_title in zip(features, features_title):
        write_to_file(
            f"rf_feature_importance/{feature_title}", "feature,RNN_ReLU,GRU", "w"
        )
        ...
        for key, value in importances.items():
            write_to_file(
                f"rf_feature_importance/{feature_title}",
                f'{key},{",".join(str(round(v, 2)) for v in value)}',
                "a",
            )
        write_to_file(
            "r_squared",
            f'{feature_title},{",".join(str(round(r, 2)) for r in r_squared)}',
            "a",
        )
